    ...
    class FrmBioController6xx
    {
        ...
        public void EnviarDigitaisInner(List<UsuarioBio> ListaUsuariosEnviar)
        {
            ...
            if (Conectar())
            {
                ...
                {
                    if (InnerAtual.TipoComBio == 0) // LFD
                    {
                    ...
                    else if (InnerAtual.TipoComBio == 1) // LC
                    {
                        ...
                        Digital1 = Utils.HexToByteArray(ListaUsuariosEnviar[index].TemplateA, 256, 512);
                        if (ListaUsuariosEnviar[index].TemplateB != "")
                            Digital2 = Utils.HexToByteArray(ListaUsuariosEnviar[index].TemplateB, 256, 512);
                    }
                        ...
                        if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            ...
                            if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                            {
                                ...
                                if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                                {
                                    ...
                                    usuarioBio.TemplateA = retornarTemplate1(Buffertemplate, InnerAtual.TipoComBio);
                                    usuarioBio.TemplateB = retornarTemplate2(Buffertemplate, InnerAtual.TipoComBio);
        ...
        private string retornarTemplate1(byte[] template, int tipoTemplate)
        {            
            switch (tipoTemplate){
                case 0:
                    return Utils.ByteToHex(template, 68, 404);                    
                case 1:
                    return Utils.ByteToHex(template, 570, 502);
                case 2:
                    return Utils.ByteToHex(template, 17, 256);
            }
            return ""; 
        }

        private string retornarTemplate2(byte[] template, int tipoTemplate)
        {
            switch (tipoTemplate)
            {
                case 0:
                    if (template.Length > 472){
                        return Utils.ByteToHex(template, 472, 404);
                    }
                    break;
                case 1:
                    if (template.Length > 570)
                    {
                        return Utils.ByteToHex(template, 570, 502);
                    }
                    break;
                case 2:
                    if (template.Length > 273)
                     return Utils.ByteToHex(template, 273, 256);
                    break;
            }
            return "";
        }
        ...
        public void CadastrarBioInner1Dedo(string numUser)
        {
            LimparlstCadastroInner();
            AtualizarInfoTelaCadLeitorInner("Conectando ao Inner...");
            AtualizarMouseCursor(Cursors.WaitCursor);
            try
            {
                if (Conectar())
                {
                    if (InnerAtual.TipoComBio == (int)Enumeradores.ModuloBio.LN)
                    {
                        MessageBox.Show("Posicione a digital!");
                        if (inserirTemplateUsr(numUser, 0) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
                            AtualizarMouseCursor(Cursors.Default);
                            return;
                        }

                        MessageBox.Show("Posicione novamete a digital!");

                        if (inserirTemplateUsr(numUser, 3) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
                            AtualizarMouseCursor(Cursors.Default);
                        ...
                        MessageBox.Show("Digital capturada!");

                    }
                    else
                    {
                        MessageBox.Show("Cadastre a digital no equipamento!");
                        if (inserirTemplateUsr(numUser, 0) == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                            MessageBox.Show("Digital capturada!");
                        else
                            MessageBox.Show("Falha na captura!");
        ...
        internal void CadastrarBioInner2Dedo(string numUser)
        {
            LimparlstCadastroInner();
            AtualizarInfoTelaCadLeitorInner("Conectando ao Inner...");
            AtualizarMouseCursor(Cursors.WaitCursor);
            try
            {
                if (Conectar())
                {
                    if (InnerAtual.TipoComBio == (int)Enumeradores.ModuloBio.LN)
                    {
                        ...
                        MessageBox.Show("Posicione novamete a primeira digital!");

                        if (inserirTemplateUsr(numUser, 1) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
                            AtualizarMouseCursor(Cursors.Default);
                        ...
                        if (inserirTemplateUsr(numUser, 2) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
                            AtualizarMouseCursor(Cursors.Default);
                        ...
                        MessageBox.Show("Posicione novamete a segunda digital!");
                    ...
                    {
                        MessageBox.Show("Cadastre a primeira digital no equipamento!");
                        if (inserirTemplateUsr(numUser, 0) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
                        ...
                        MessageBox.Show("Cadastre a segunda digital no equipamento!");
                        if (inserirTemplateUsr(numUser, 1) != (int)Enumeradores.Retorno.RET_COMANDO_OK)
                        {
                            MessageBox.Show("Falha na captura!");
                            EasyInner.FecharPortaComunicacao();
        ...
        private int inserirTemplateUsr(String Usr, int numTpl)
        {
            int Ret = -1;
            int timeout;

            //Solicita inserção
            Ret = EasyInner.RequisitarCadastrarUsuarioLeitorInnerBio(InnerAtual.Numero, InnerAtual.TipoComBio, (byte)numTpl, Usr);

            Thread.Sleep(2000);
            timeout = (int)EasyInner.RetornarSegundosSys() + 10;
            do
            {
                //Retorna resultado inserção          
                Application.DoEvents();
                Ret = EasyInner.RespostaCadastrarUsuarioLeitorInnerBio(InnerAtual.Numero);

            } while ((Ret != (int)Enumeradores.Retorno.RET_COMANDO_OK) && ((int)EasyInner.RetornarSegundosSys() <= timeout));
