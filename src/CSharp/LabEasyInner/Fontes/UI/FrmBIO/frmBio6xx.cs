...
using LMLeitorUSB.Comunicacao;

namespace EasyInnerSDK.UI.FrmBIO
{
    public partial class frmBio6xx : Form
    {
        public delegate void delAtualizarcheck(int item);
        public delegate void delExibemensagem(string item);
        public delegate void delDesmarcarCheck(); 
        ...
        private InterfaceBio controleLM = new InterfaceBio();
        ...
        public frmBio6xx(frmMain pai)
        {
            InitializeComponent();
            this.MdiParent = pai;
            controleLM.eventAmostra += new InterfaceBio.delAmostraRecebida(controleLM_eventAmostra);
            controleLM.eventDigital += new InterfaceBio.delDigitalRecebida(controleLM_eventDigital);
            controleLM.eventStatusConexao += new InterfaceBio.delStatusConexao(controleLM_eventStatusConexao);
            Show();
        }

        void controleLM_eventStatusConexao(ref bool status, ref string mensagem)
        {
            if (!status) {
        ...
        void controleLM_eventDigital(ref int numeroDigital, ref string digital)
        {
            UserBio.Usuario = txtCartaoCap.Text;
            if (numeroDigital == 1)
            {
                UserBio.TemplateA = digital;
                MessageBox.Show("Capturar 2 digital", "Capturar digital pelo leitor USB");
                controleLM.cadastrarBio(2);
                this.Invoke(new delDesmarcarCheck(this.desmarcar));
            }
            else {
                UserBio.TemplateB = digital;
                AcessoTemplates.InserirTemplateBD(UserBio, 2); //Leitor LM    
                MessageBox.Show("Digital capturada com sucesso");
                this.Invoke(new delExibemensagem(this.exibeMensagem), new object[] { "Digital cadastrada com sucesso" });
                fecharPortaLM();
            }
        }

        void controleLM_eventAmostra(ref int amostra, ref string mensagem)
        {
            this.Invoke(new delExibemensagem(this.exibeMensagem), new object[] { mensagem });
            if (amostra != 0)
            {
                this.Invoke(new delAtualizarcheck(this.atualizarChk), new object[] { amostra });
            }else {          
                    if (mensagem.Contains("Erro")) //Mensagem de erro fecha tudo
                    {
                        fecharPortaLM();
                    }                                                                          
            }
        ...
        void atualizarChk(int amostra) {
            switch (amostra)
            {
                case 1:
                    chkCaptura1.Checked = true;
                    break;
                case 2:
                    chkCaptura2.Checked = true;
                    break;
                case 3:
                    chkCaptura3.Checked = true;                    
                    break;
            }
            Application.DoEvents();
        }
        private void fecharPortaLM()
        {            
   
            this.Invoke(new delDesmarcarCheck(this.desmarcar));
            controleLM.fecharComm();
            
        }

        private void desmarcar() {
            chkCaptura1.Checked = false;
            chkCaptura2.Checked = false;
            chkCaptura3.Checked = false;
        }

        private void frmBio6xx_Load(object sender, EventArgs e)
        {
            cboModuloBio.SelectedIndex = 0;
            lblModuloBio.Text = cboModuloBio.SelectedIndex == 0 ? "Modulo bio LN" : (cboModuloBio.SelectedIndex == 1 ? "Modulo bio LC" : "Modulo bio LM");
        ...
        private void InicializarDispositivosUSB()
        {
            ...
            //cboDispositivos.Items.Add("Leitor LM");
            ...
            if (controleLM.equipamentoConectado())
            {
                cboDispositivos.Items.Add("Leitor LM");
                gpbCapturasLC.Visible = false;
                gpbConfHamster.Visible = true;
        ...
        private void cboDispositivos_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cboDispositivos.Text == "Leitor LM") {
                gpbCapturasLC.Text = "Leitor LM";
                cboModuloBio.SelectedIndex = 2;
            }
        }

        private void btnCapturar_Click(object sender, EventArgs e)
        {
            UserBio = new UsuarioBio();
            AcessoTemplates = new DAOUsuariosBio();
            BioController6xx = new FrmBioController6xx(this, InnerAtual);
            ...
            else if (cboDispositivos.Text == "Leitor LC")
            {
                MessageBox.Show("Capturar 1 digital", "Capturar digital pelo leitor USB");
            ...
            else  //Senão, então é LM
            {
                if (controleLM.initComm())
                {
                    MessageBox.Show("Capturar 1 digital", "Capturar digital pelo leitor USB");
                    controleLM.cadastrarBio(1);
                }
                else
                    MessageBox.Show("Não foi possível iniciar o leitor USB");
        ...
        private void btnCadastrarDigital1dedo_Click(object sender, EventArgs e)
        {
            UsuarioBio UserBio = new UsuarioBio();
            BioController6xx = new FrmBioController6xx(this, InnerAtual);
            BioController6xx.CadastrarBioInner1Dedo(txtCartaoCadInner.Text);
        }

        private void btnCadastrarDigital2dedo_Click(object sender, EventArgs e)
        {
            UsuarioBio UserBio = new UsuarioBio();
            BioController6xx = new FrmBioController6xx(this, InnerAtual);
            BioController6xx.CadastrarBioInner2Dedo(txtCartaoCadInner.Text);
        }
        ...
        private void tbpManutencaoTemplates_Click(object sender, EventArgs e)
        {

        }

        private void dgvDigitaisInner6xx_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
