...
{
    // Cria um novo Objeto Inner com valores padrão.
    Inner objInner = null; // Declaração do objInner fora do loop
    for (int numero = 22; numero <= 24; numero++) // Modifique o intervalo conforme necessário
    {
        objInner = new Inner
        {
            Biometrico = true, // Define se o Inner tem biometria, valor de exemplo.
            Catraca = true, // Define se é catraca, valor de exemplo.
            EstadoAtual = Enumeradores.EstadosInner.ESTADO_CONECTAR,
            EstadoTeclado = Enumeradores.EstadosTeclado.TECLADO_EM_BRANCO,
            Numero = numero, // Número do Inner atualizado no loop
            QtdDigitos = 6, // Quantidade de dígitos padrão, valor de exemplo.
            Teclado = false, // Define se tem teclado, valor de exemplo.
            Lista = true, // Configura a lista Off padrão, valor de exemplo.
            ListaBioSemDigital = false, // Define se tem lista sem digital, valor de exemplo.
            TipoLeitor = 3, // Tipo de leitor padrão, valor de exemplo.
            Identificacao = 1, // Identificação padrão.
            Verificacao = 0, // Verificação padrão.
            DoisLeitores = true, // Dois leitores padrão, valor de exemplo.
            Acionamento = 1,
            VariacaoInner = 0,
            CatInvertida = true, // Cat invertida padrão, valor de exemplo.
            PadraoCartao = 1, // Padrão de cartão, valor de exemplo.
            TipoConexao = 2, // Tipo de conexão padrão, valor de exemplo.
            Porta = 3575, // Porta padrão, valor de exemplo.
            TipoComBio = 0, // Tipo de comunicação biométrica, valor de exemplo.
            CartMaster = false, // Cartão master habilitado, valor de exemplo.
            Master = "" // Cartão master padrão, valor de exemplo.
        };

        // Adiciona o Inner padrão à lista antes da verificação
        if (!ListInners.ContainsKey(objInner.Numero))
        {
    ...
    // Cria um novo Objeto Inner com os dados informados em tela.
    Inner objInnerNovo = new Inner();
    if (UpdateDisplay.getInfoInner() == false)
    {
        return;
    }
    objInnerNovo.Biometrico = UpdateDisplay.Biometrico;

    // Se catraca
    objInnerNovo.Catraca = UpdateDisplay.Acionamento != (byte)Enumeradores.Acionamento.Acionamento_Coletor;

    // Seta nas configurações do Inner os dados informados em tela
    objInnerNovo.EstadoAtual = Enumeradores.EstadosInner.ESTADO_CONECTAR;
    objInnerNovo.EstadoTeclado = Enumeradores.EstadosTeclado.TECLADO_EM_BRANCO;
    objInnerNovo.Numero = UpdateDisplay.NumInner;
    objInnerNovo.QtdDigitos = UpdateDisplay.QtdDigitos;
    objInnerNovo.Teclado = UpdateDisplay.Teclado;
    objInnerNovo.Lista = UpdateDisplay.ListaOff;
    objInnerNovo.ListaBioSemDigital = UpdateDisplay.ListaSemDigital;
    objInnerNovo.TipoLeitor = UpdateDisplay.TipoLeitor;
    objInnerNovo.Identificacao = (byte)(UpdateDisplay.Identificacao ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO);
    objInnerNovo.Verificacao = (byte)(UpdateDisplay.Verificacao ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO);
    objInnerNovo.DoisLeitores = UpdateDisplay.DoisLeitores;
    objInnerNovo.VariacaoInner = 0;
    objInnerNovo.Acionamento = UpdateDisplay.Acionamento;
    objInnerNovo.CatInvertida = UpdateDisplay.CatInvertida;
    objInnerNovo.PadraoCartao = UpdateDisplay.PadraoCartao;
    objInnerNovo.TipoConexao = UpdateDisplay.TipoConexao;
    objInnerNovo.Porta = UpdateDisplay.Porta;
    objInnerNovo.TipoComBio = UpdateDisplay.ModuloBio;
    objInnerNovo.CartMaster = UpdateDisplay.HabCartaoMaster;
    objInnerNovo.Master = UpdateDisplay.CartaoMaster;

    foreach (Inner InnerAtual in ListInners.Values)
    {
        if (objInnerNovo.Numero == InnerAtual.Numero)
        {
        ...
        private void PASSO_ESTADO_ENVIAR_CONFIGMUD_ONLINE_OFFLINE(Inner InnerAtual)
        {
            ...
            {                
                ...
                EasyInner.DefinirMensagemPadraoMudancaOnLine(1, "HMDI-Tec.info");
        ...
        private void PASSO_ESTADO_ENVIAR_MSG_PADRAO(Inner innerAtual)
        {
            try
            {
                ...
                if (EasyInner.EnviarMensagemPadraoOnLine(innerAtual.Numero, 1, "HMDI-Tec.info") == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                {
        ...
        private void PASSO_ESTADO_MONITORA_GIRO_CATRACA(Inner InnerAtual)
        {
            try
            {
                ...
                if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                {
                    ...
                    {
                        UpdateDisplay.AtulizarLabelDados("Nao girou a catraca!");
                    }
                    else if (Bilhetes.Origem == (int)Enumeradores.Origem.GIRO_DA_CATRACA_TOPDATA)
                    {
                        Bilhetes.Direcao = (Bilhetes.Complemento - Convert.ToInt16(InnerAtual.CatInvertida) == 0) ? "entrada" : "saída";
                        Cartao.Append(InnerAtual.BilheteOnline.Cartao.ToString());
                        string data = $"{Bilhetes.Dia:D2}/{Bilhetes.Mes:D2}/{Bilhetes.Ano}";
                        string hora = $"{Bilhetes.Hora:D2}:{Bilhetes.Minuto:D2}:{Bilhetes.Segundo:D2}";
                        string tipo = $"{Bilhetes.Direcao}";
                        string mensagem = $"Girou a catraca para {Bilhetes.Direcao}. Cartão: {Cartao} Bilhete: {Bilhetes.Origem}, Bilhete Complemento: {Bilhetes.Complemento}, Data: {Bilhetes.Dia:D2}/{Bilhetes.Mes:D2}/{Bilhetes.Ano}, Hora: {Bilhetes.Hora:D2}:{Bilhetes.Minuto:D2}:{Bilhetes.Segundo:D2}.";
                        DAOConexao.SalvarMonitoramento(Cartao.ToString(), data, hora, tipo);
                        UpdateDisplay.AtulizarLabelDados(mensagem);
                    }
                    UpdateDisplay.FuncoesCatraca(true, true);
                    InnerAtual.EstadoAtual = Enumeradores.EstadosInner.ESTADO_ENVIAR_MSG_PADRAO;
                }
