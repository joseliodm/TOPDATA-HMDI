...
namespace EasyInnerSDK.DAO
{
    class DAOUsuariosBio
    {
        ...
        public List<UsuarioBio> ConsultarUsuariosBio(int TipoComBio)
        {
            try
            {
                string ComBio = definirBaseBio(TipoComBio);
                DbCommand Command = DAOConexao.ConectarBase().CreateCommand();
        ...
        private static string definirBaseBio(int TipoComBio)
        {
            string ComBio = TipoComBio == 0 ? "UsuariosBio" : TipoComBio == 1 ? "UsuariosBioLC" : "UsuariosBioLM";
            return ComBio;
        }
        public bool ExisteUsuarioBio(string usuario, int TipoComBio)
        {
            string tabelaBio = definirBaseBio(TipoComBio);
            DbCommand Command = DAOConexao.ConectarBase().CreateCommand();
